...
uint16_t TimeOfToggle = START_SPEED; //lebo jedno toggle za 9375us je 1ot·Ëka/min˙tu pri 1/16 krokovanÌ
							//jeden krok za 94*100*2 us = 18,8ms
							//3191,5 krokov/sekundu
uint16_t TimeOfToggleDecrement = 4; //toto ËÌslo znamen· o koæko sa bude zmenöovaù r˝chlosù kaûd˝ch 0,5 s
									...
									//Kaûd˝ch 1s sa zmenöÌ Ëas jednÈho kroku o 1ms
	...
	if(Init){
		...
		if(Auto == 1){
			if(MicroSteppingChange){
				//SetAngle(((recv_end_angle-recv_start_angle)/2)+recv_start_angle);//nastav stred uhla novÈho novÈho rozsahu
	...
	if(SetAngleFinished){
		...
		if(!(interupt_counter_auto%TimeOfToggle))
		{
			if(Steps >= end_step || Steps <= start_step){
				interupt_counter_auto = 0;
				TimeOfToggle = START_SPEED; //lebo jedno toggle za 9375us je 1ot·Ëka/min˙tu pri 1/16 krokovanÌ
...
//funkcia ktor· sa spuöta v EasyStepper a sl˙ûi na nastavenie polohy medzi hraniËn˝mi polohami pre auto m¥d
...
void set_recv_data()
{
	if(Init && Finish)
	{
		...
		else // auto mode
		{
			...
			TimeOfToggleDecrement = TimeOfToggleDecrement_double;//ak je v GUI nastavenÈ 0.2ms, prijmem ËÌslo 2. MusÌm to podeliù 10 a vyn·sobiù (1/0.2) = recv_speed/2

			SetAngleFinished = 0;//nastav stred uhla novÈho novÈho rozsahu
