...
void spi_set_current(uint8_t current) //funkcia na nastavenie prúdového odberu na drive cez SPI.
{
...
uint8_t  WriteSPI1(uint8_t  txAddr, uint8_t  txData) //funkcia na zápis dát na SPI zbernicu
{
 uint8_t rxData;
...
uint8_t ReadSPI1(uint8_t  txAddr) //funkcia na èítanie dát na SPI zbernici
{
 uint8_t   rxData;
...
uint8_t EnableDisable(int state) //funkcia zapína a vypína mostík aby bolo možné vykona nastavenie parametrov motora
{
...
void initSPI1(void)//funkcia na inicializáciu SPI1 a SPI pinov PA5, PA6, PA7.
{
...
void initCS_Pin(void) //funkcia na inicializáciu pinu SPI1_CS - PA4
{
	GPIO_InitTypeDef GPIO_InitStructure;
